...
public class OrderMobileMapperProfile : Profile
{
    public OrderMobileMapperProfile()
    {
        ...
        CreateMap<Order, ListOrderDto>()
            ...
            .ForMember(dest => dest.TotalPrice, opt => opt.MapFrom(src => src.OrderProducts.Sum(x => x.Price * x.Quantity) + src.DeliveryFare - src.Discount))
            .ForMember(dest => dest.RepresentativeName, opt => opt.MapFrom(src => src.Representative.UserInfo.Name))
            .ReverseMap();
        #endregion
        ...
        CreateMap<Order, DetailsOrderDto>()
            ...
            .ForMember(dest => dest.Price, opt => opt.MapFrom(src => src.OrderProducts.Sum(op => op.Price * op.Quantity)))
